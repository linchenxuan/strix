...
func TestDimensionClone(t *testing.T) {
	var nilDim Dimension
	if nilDim.Clone() != nil {
		t.Fatal("expected nil clone from nil dimension")
	}
	...
	cloned := origin.Clone()
	if len(cloned) != len(origin) {
		t.Fatalf("expected cloned len %d, got %d", len(origin), len(cloned))
	}
	if cloned["host"] != "server1" || cloned["region"] != "us-west" {
		t.Fatal("unexpected cloned dimension values")
	}

	cloned["host"] = "server2"
	if origin["host"] != "server1" {
		t.Fatal("modifying cloned dimension should not affect origin")
	}
}
