...
func TestSetMetricsReporters(t *testing.T) {
	...
	// 验证内部持有的是切片副本，外部修改不应影响全局配置
	reporters[0] = nil
	if _Reporters[0] == nil {
		t.Fatal("Expected internal reporters to be copied, got nil reporter")
	}
	...
	// 二次设置应触发 panic（只允许初始化一次）
	defer func() {
		if r := recover(); r == nil {
			t.Fatal("Expected panic on second SetReporters call")
		}
	}()
	SetReporters([]Reporter{NewMockReporter()})
}
